        ...
        {sent ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 16 }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>
              Îți vom răspunde la <strong style={{ color: 'white' }}>{email}</strong> în maxim 24 de ore.
            </p>
          </div>
        ) : (
          <div style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: 16, padding: 36 }}>

            {/* Tip solicitant */}
            <div style={{ marginBottom: 24 }}>
              <label style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, display: 'block', marginBottom: 10 }}>TIPUL SOLICITĂRII</label>
              <div style={{ display: 'flex', gap: 12 }}>
                <button
                  onClick={() => setType('persoana')}
                  style={{
                    flex: 1, padding: '10px', borderRadius: 8, border: `1px solid ${type === 'persoana' ? '#0ea5e9' : 'rgba(255,255,255,0.1)'}`,
                    background: type === 'persoana' ? 'rgba(14,165,233,0.1)' : 'transparent',
                    color: type === 'persoana' ? '#0ea5e9' : 'rgba(255,255,255,0.5)',
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: '10px', borderRadius: 8, border: `1px solid ${type === 'companie' ? '#0ea5e9' : 'rgba(255,255,255,0.1)'}`,
                    background: type === 'companie' ? 'rgba(14,165,233,0.1)' : 'transparent',
                    color: type === 'companie' ? '#0ea5e9' : 'rgba(255,255,255,0.5)',
                    cursor: 'pointer', fontSize: 13, fontWeight: 600
                  }}
                >
                  🏢 Companie
            ...
            {type === 'companie' && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, display: 'block', marginBottom: 6 }}>PERSOANĂ DE CONTACT</label>
                  <input
                    value={company}
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, display: 'block', marginBottom: 6 }}>NUMĂR ANGAJAȚI</label>
                  <select
                    value={employees}
                    onChange={e => setEmployees(e.target.value)}
                    style={{ width: '100%', background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: 8, padding: '10px 12px', color: 'white', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                  >
                ...
                <div style={{ background: 'rgba(14,165,233,0.06)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: 10, padding: '14px 16px', marginBottom: 16, fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>
                  🏢 <strong style={{ color: 'white' }}>Soluție dedicată pentru companii</strong><br />
            ...
            {/* Mesaj */}
            <div style={{ marginBottom: 24 }}>
              <label style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, display: 'block', marginBottom: 6 }}>MESAJ</label>
              <textarea
                value={message}
                onChange={e => setMessage(e.target.value)}
                rows={4}
                placeholder={type === 'companie' ? 'Descrie pe scurt nevoile companiei tale...' : 'Cu ce te putem ajuta?'}
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: 8, padding: '10px 12px', color: 'white', fontSize: 14, outline: 'none', resize: 'none', fontFamily: 'sans-serif', boxSizing: 'border-box' }}
              />
            </div>
            ...
            <button
              ...
              style={{
                ...
                background: name && email && message ? 'linear-gradient(135deg,#0ea5e9,#6366f1)' : 'rgba(255,255,255,0.1)',
